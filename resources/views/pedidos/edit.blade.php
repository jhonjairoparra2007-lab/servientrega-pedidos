...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--gris-borde); padding-bottom: 1rem; margin-bottom: 1.5rem;">
        <h2 style="color: var(--rojo-servientrega); margin: 0;">Editar Pedido #{{ $pedido->id }}</h2>
    ...
    <form action="{{ route('pedidos.update', $pedido->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" class="form-control" value="{{ old('title', $pedido->title) }}" required>
        </div>
        
        <div class="form-group" style="display: flex; gap: 1rem;">
            <div style="flex: 1;">
                <label class="form-label">Precio Declarado (USD) *</label>
                <input type="number" step="0.01" name="price" class="form-control" value="{{ old('price', $pedido->price) }}" required>
            </div>
            <div style="flex: 1;">
                <label class="form-label">Categoría</label>
                <input type="text" name="category" class="form-control" value="{{ old('category', $pedido->category) }}">
            </div>
        </div>
        
        <div class="form-group">
            <label class="form-label">URL de la Imagen (Opcional)</label>
            <input type="url" name="image" class="form-control" value="{{ old('image', $pedido->image) }}" placeholder="https://ejemplo.com/imagen.jpg">
        </div>
        
        <div class="form-group">
            <label class="form-label">Descripción Detallada</label>
            <textarea name="description" class="form-control">{{ old('description', $pedido->description) }}</textarea>
        </div>
        
        <div style="margin-top: 2rem;">
            <button type="submit" class="btn" style="width: 100%; padding: 1rem; font-size: 1.1rem; background-color: #2563eb;">Actualizar Pedido</button>
        </div>
    </form>
</div>
@endsection
